...
class Wildfire:
    ...
    def move(self, board, n, d):
        ...
        Returns the point where Wildfire wants to move.
        """
        ...
        otherForces = self.findForces(board, board.otherNumber(n),
                                      self.MAX_FORCES, d)

        possibleMoves = [p for p in board.openPoints()
                         if p not in self.findCasualChecks(board, n)]
        if otherForces:
    ...
    def winningMove(self, board, n):
        """ Checks if there is a winning move for player n
        and if there is, returns it"""
    ...
    def findCasualChecks(self, board, n):
        ...
        for force in possibleForces:
            openSpots = filter(lambda p: board.pointToValue(p) == 0,
                               board.lineToPoints(force))
            checks += openSpots
        ...
        for force in possibleForces:
            ...
            for moves in [openSpots, openSpotsRev]:
                ...
                if movesLeft == self.MAX_FORCES:
                    d.displayProgress(self.step, (50*i)/(len(possibleForces)))
                    i += 1
    ...
    def evaluatePosition(self, board, n):
        """ Evaluates a certain board position
    ...
    def findDefensiveMoves(self, board, n, otherForces, d):
        ...
        for i in range(len(otherForces)):
            ...
            otherForces[i] += [check for check in defensiveChecks
                               if check not in startingChecks]

        defensiveMoves = [p for p in board.openPoints()
                          if self.isDefensive(p, otherForces)]

        return defensiveMoves

    def isDefensive(self, p, totalDefensiveMoves):
        """ returns whether p is a defensive move
        for all possible opponent's forces"""
